...
3. [Ветки](#3-ветки)
    - [Создание и переключение](#31-создание-и-переключение)
...
- `git rm -r <src> --cached` - удаляет из индекса, но оставляет в рабочей дирректории и делает файл/директорию неотслеживаемым/ой при попытке удалить файл, который несохранен нигде git выдает предупрежнения для предотвращения безвозвратной потери данных. Это можно игнорировать,  используя флаг -f (forse)
...
-`git branch -m <main>` - ереименовываем основную ветку если нужно, *main* это классика, иногда это может быть *trunck* | *development*
