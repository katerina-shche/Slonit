...
3. [Ветки](#3-ветки)
    ...
    - [Удаление веток](#310-удаление-веток)
    - [3.11 История переключений веток: лог ссылок reflog](#311-история-переключений-веток-лог-ссылок-reflog)
...
-`git branch -d <branchname>` - удаляет ветку (если она уже слита с другой, если нет - появится предупреждение от git `the branch <branchname> is not fully merged`, так как после удаления ветки, все коммиты в ней становятся недостижимыми и современем тоже удаляются автоматически)
-`git branch -D <branchname>` - удалит ветку даже если она не слита, и все коммиты из нее будут безвозвратно утеряны через какое-то время, но первое время они еще висят, и если известен номер, то можно поставить на них новую ветку с тем же именем - что по сути будет восстановлением .
-`git branch <branchname> <commitid>` - пока коммиты ну удалены git автоматически эта команда сможет поставить на них ветку
