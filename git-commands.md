...
### 3.2. Команда checkout при незакоммиченных изменениях

При незакомиченных изменениях при попытке перехода на другую ветку c помощью `git checkout <existingbranchname>` git выдаст ошибку 
```zsh
